...
const LINTED_EXTENSIONS = [".ts", ".js", ".mjs"];
const UNLINTED_DIRS = ["node_modules", ".git", "dist", "coverage"];
...
function isLinted(target: string): boolean {
  ...
  if (target.split(sep).slice(0, -1).some((dir) => UNLINTED_DIRS.includes(dir))) return false;
  if (target.startsWith(join(".claude", "worktrees") + sep)) return false;
  return LINTED_EXTENSIONS.some((extension) => target.endsWith(extension));
}
