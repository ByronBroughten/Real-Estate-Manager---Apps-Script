...
- **`02_SpreadsheetRaw` is raw-index Sheets API access only** — whole rows/columns by GID + index, sheet properties, and `UniformRow`. It never resolves a column by name or `columnId`; that's `03_SpreadsheetIndexed`/`04_SpreadsheetNamed`'s job, including generating/updating `01_generatedConfigs` (`SheetConfigOperator`/`ColumnConfigOperator`, tier 04) — config maintenance is no longer Raw's responsibility.
