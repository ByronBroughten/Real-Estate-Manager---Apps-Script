...
- **`src/00_*` through `src/06_API/` are a project-agnostic framework** for building typed, structured apps on top of Google Sheets + Apps Script. The Raw tier talks to the Sheets Advanced API purely in terms of raw row/column indexes and sheet properties; the Indexed and Named tiers resolve the typed schema on top of that by index/name. A dedicated Operators tier (`05_Operators`) sits above Named and owns generating/updating `01_generatedConfigs` from the live "Sheet Config"/"Column Config"/"Value Config" sheets — maintaining generated data is neither Raw's nor Named's job. `06_API` sits on top as a generic endpoint-dispatch layer (`Api`, `RunnerEndpointHandler`/`RunnerEndpointHandlerBase`) that routes sheet-edit events to registered endpoints by column name. Nothing in these folders should reference real-estate concepts (properties, leases, tenants, etc.). If you're adding something reusable that any Sheets-backed app would want, it belongs here.
...
**What a runner, a selector, a stem, a run status and a run state *are* is defined in [`CONTEXT.md`](./CONTEXT.md)** — read that first; this section is only how the dispatch is built.

An action-row checkbox is wired to an endpoint by its **column name suffix**, declared in `spreadsheetConfig`:

- **Runner** (`runnerEndpointSuffix`, `TimeLastRan`) — fires only when the box is checked; the endpoint receives just the spreadsheet props. Its one status column is declared alongside it: `runStatusEndpointSuffix` (`RunStatus`).
- **Selector** (`selectorEndpointSuffix`, `Select`) — fires on check **and** uncheck, and receives `isSelected`.

`Endpoints` is the intersection of two `Partial<Record<…>>`s keyed by those suffix-filtered unions, so the two families have different signatures and a key that is not a real `ColumnFullName` is a compile error. Each union is additionally intersected with the value filter its family requires — runners with `ColumnFullName<"string", false>`, selectors with `ColumnFullName<"boolean", false>` — so a `...Select` column that isn't actually a checkbox fails to compile instead of silently no-opping at runtime. Register endpoints with a plain `: Endpoints` annotation — **not** `makeStructuredConfig`, which lets an unknown key through (see STYLE.md, "Type modeling").

**A runner is identified by its stem, not by two column names.** `RunnerStem<SN>` is the set of column-name prefixes on a sheet that carry both runner suffixes — `RunnerStem<"spreadsheetControls">` is `"fillRowIds" | "syncConfigSheetRows"` and `RunnerStem<"occupancy">` is `"buildLedger" | "updateTerms"`, while a sheet carrying neither suffix resolves to `never`. Completeness is the point: "the sheet has at least one column of each suffix" would let a handler be assembled from two unrelated endpoints' status columns. `RunnerEndpointHandlerBase<SN, ST>` takes one stem and derives both column names as getters, so a mismatched pair is unforgeable rather than merely validated — and the endpoint column *is* the time-last-ran column, one field rather than two that could disagree. `SheetNameWithRunnerColumns` (in `SheetNameGroups.ts`, derived from config rather than hand-listed) constrains which sheets can host a handler at all; the endpoint *map* is unaffected by it, so occupancy's registered runner endpoint keeps compiling — it simply has no handler yet.
...
These are the **architecture** words — the layering vocabulary. The domain words an operator would recognise (runner, selector, stem, run status, run state) live in [`CONTEXT.md`](./CONTEXT.md) instead.
