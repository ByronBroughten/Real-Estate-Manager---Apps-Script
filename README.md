...
- **Runner** (`runnerEndpointSuffix`, `TimeLastRan`) — a run button. Fires only when the box is checked; the endpoint receives just the spreadsheet props. Its two status columns are declared alongside it: `runSucceededEndpointSuffix` (`LastRanSucceeded`) and `errorMessageEndpointSuffix` (`ErrorMessage`).
...
**A runner is identified by its stem, not by three column names.** `RunnerStem<SN>` is the set of column-name prefixes on a sheet that carry all three runner suffixes — `RunnerStem<"spreadsheetControls">` is `"fillRowIds" | "syncConfigSheetRows"`, and `RunnerStem<"occupancy">` is `never`, because occupancy's `updateTerms`/`buildLedger` trios are incomplete. Completeness is the point: "the sheet has at least one column of each suffix" would let a handler be assembled from two unrelated endpoints' status columns. `RunnerEndpointHandlerBase<SN, ST>` takes one stem and derives all three column names as getters, so a mismatched trio is unforgeable rather than merely validated — and the endpoint column *is* the time-last-ran column, one field rather than two that could disagree. `SheetNameWithRunnerColumns` (in `SheetNameGroups.ts`, derived from config rather than hand-listed) constrains which sheets can host a handler at all; the endpoint *map* is unaffected by it, so occupancy's registered runner endpoint keeps compiling — it simply has no handler yet.
