...
| `04_SpreadsheetNamed`   | Config-dependent, **name**-based API — the one most application code should use (`SheetNamed`, `ColumnNamed`, `DataRowNamed`, `*SchemaNamed`). Also owns `SheetConfigNamed`/`ColumnConfigNamed`, which read the real "Sheet Config"/"Column Config" sheets by name and regenerate tier 02's trait files — maintaining generated data is a Named-tier job now, not Raw's. | `00`–`03`        |
