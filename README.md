...
- **Indexed converts.** `RowIndexed.delete` clears instead of deleting when it would empty the sheet. `RowIndexed.clearValues` writes `""` into every configured non-formula column — the id column and checkbox columns included, so a cleared checkbox reads `""` rather than `false` — mirrors those blanks into local state, releases the row's reservation, and never touches a formula cell. `RowIndexed.isBlank` is the emptiness test, `SheetIndexed.hasNoData` its sheet-level counterpart, and `SheetIndexed.DELETE_ALL_DATA_ROWS()` is the one way to empty a sheet: it deletes every data row below the top one and clears the top one, so which row survives a wipe is predictable rather than a consequence of loop order.
...
**Use absolute addressing when you need to name a set of columns that cuts across sheets** — "every column whose value name is `boolean` and which isn't a formula" is inexpressible as a `<SN, CN>` pair, because the constraint doesn't live within one sheet. That set is what lets `CheckboxColumnOperator` be attachable only to a real checkbox column, and what makes the endpoint-name unions checkable.
...
- **Operator** — a class based on a Named base class that adds methods suited to one particular data structure, reaching its subject through a getter rather than inheriting it (see STYLE.md). Config maintenance is one family of Operators, not the definition: `SheetConfigOperator`/`ColumnConfigOperator`/`ValueConfigOperator` each read a real Config sheet by name and regenerate the matching tier-01 config file, and `ConfigOrchestrator` coordinates several into one flow. Maintaining generated data is an Operator-tier job, not Named's. `CheckboxColumnOperator` is the non-config kind — a column operator that can only be attached to a non-formula boolean column.
...
- **Schema** — type/validation logic layered on top of Config data. Two unrelated families share the word: the spreadsheet schema (`SpreadsheetSchema`/`SheetSchema`/`ColumnSchema`, see "The schema classes") and the value schema (`ValueSchema`, for validating a single cell's value). **Every cell is tri-state: `Value<VN>` always includes `""`** — a `boolean` column reads `boolean | ""`, a `number` column `number | ""` — because an untouched cell is empty, not defaulted. Code that branches on a column's value has to say what empty means rather than assuming the base type.
  ...
  - **The checkbox is the trap.** A never-ticked checkbox reads `""`, not `false`. So `column.value(rowIndex) === true` over a selector column throws on every row nobody has touched, and a checked-row filter has to read `valueOrEmpty` — which is what `CheckboxColumnOperator.rowIndexesChecked` does. The same goes for every drift check against a config sheet, where the blank *is* the drift being detected.
