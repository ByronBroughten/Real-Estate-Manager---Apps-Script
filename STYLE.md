...
- **Reach for the empty-allowing read only when the caller handles blank.** The unmarked accessors already validate at the read line: `value`/`valueArr` exclude `""` and throw naming the blank cell. Choosing `valueOrEmpty`/`valueArrOrEmpty` instead is a claim that this call site has decided what a blank means — an optional field, a checkbox nobody has ticked, a drift check where the blank *is* the drift. Don't read through the marked form and defer the check to a manual guard closer to where the value gets used; reading-and-validating in one step means a value can never be used unvalidated in between, and keeps every field in an object literal validated the same way. `ColumnConfigOperator.newColumnConfigs()` reads `columnId`/`sheetGid`/`isFormula`/`header`/`valueTitle` all via `col.x.value(rowIndex)` for exactly that reason, while the drift comparisons a few methods up read `valueOrEmpty` because a blank config cell is what they exist to catch.
