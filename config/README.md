...
General tooling for TypeScript projects: an ESLint flat-config preset, the prettier config, a base tsconfig and a doc linter. Plain JS, no build step; one version of each peer (eslint, prettier, typescript) is needed only for the export that uses it.
...
- **Links and anchors** resolve, in `AGENTS.md`, `CLAUDE.md`, `CONTEXT.md`, `README.md`, and every `docs/` file at the repo root and under `packages/<name>/`. A link into a `packages/<name>/` folder the repo doesn't track is external and not checked, so a root doc can point at clones that exist only locally.
- **`--published <dir>`** names the package whose `docs/`, `CONTEXT.md` and `README.md` ship on their own, so they link only inside it: `packages/<name>`, or `.` for the repo root, either relative to the repo root wherever the bin runs. Anything else is an error, and without the flag no package is held to the rule.
