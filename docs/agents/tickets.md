...
- **A platform capability `src/` has never called gets its wrapper's file named, and a Done-when box for the fake.** The ticket that first reaches an Apps Script global — a toast, a new trigger kind — never leaves the wrapper's home or the fake's update to the implementer.
- **A spec never places a platform call or platform type outside `src/00_Source/GoogleSheets/` and the entry points.** It names the neutral type or return value that carries the capability across.
