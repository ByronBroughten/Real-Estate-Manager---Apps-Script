...
- **`src/chores/`** — the framework's generic chores, kept and listed in every package: `addMissingColumnIds`, `fillMissingRowIds`, and `ensureConfigSheetFloor`. A package chore with one of their names stops the runner. A twice-a-year repair belongs here, or in a package's own durable home, rather than earning a column and a checkbox on a sheet.
- **`dev/chores/`** — the dev spreadsheet's own home. `buildDevFixtures` is the dev fixture's recipe (its data and builder sit in `devFixtures/`, which the runner does not list): it creates missing fixture tabs, and a rebuild is deleting the tab and rerunning. It refuses any spreadsheet but the dev one, and reads its layout from `dev/generated/`, which `dev:chore` installs.
