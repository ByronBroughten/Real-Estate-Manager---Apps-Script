...
- **`src/chores/`** — durable chores, kept. `addMissingColumnIds`, `fillMissingRowIds`, and `ensureConfigSheetFloor` are the generic three. `buildDevFixtures` is the dev fixture's recipe (its data and builder sit in `devFixtures/`, which the runner does not list), and it refuses any spreadsheet but the dev one. A twice-a-year repair belongs here rather than earning a column and a checkbox on a sheet.
