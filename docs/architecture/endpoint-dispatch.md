...
An endpoint is one entry keyed by the column whose action-row checkbox triggers it, and `EndpointRun` owns its run: selection, running state, action, outcome, feedback. The rules for writing one: [`src/businessEndpoints/AGENTS.md`](../../src/businessEndpoints/AGENTS.md) and [`src/06_API/AGENTS.md`](../../src/06_API/AGENTS.md).
...
`Endpoints` is the app's map: a key on one of the four config sheets is a type error, because the framework owns those entries (`frameworkEndpoints`). `Api` spreads the framework's entries last into an `EndpointsDispatched`, so a collision still resolves to the framework at runtime.
