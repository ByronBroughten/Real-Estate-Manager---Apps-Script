...
**One endpoint, one file.** `businessEndpoints.ts` stays a registry: each entry's body lives in `businessEndpoints/<name>.ts` as a single exported `Endpoint<"thatSheet">` the registry imports and assigns to its key, so an endpoint's size never crowds its neighbours and the plain `: Endpoints` annotation still catches a key naming a column that doesn't exist. `businessEndpoints/buildLedger.ts` is the reference; the existing update-terms entry has deliberately not been migrated, since the pattern is being proven on a new endpoint before it is applied to a working one. An endpoint is module-private helpers around that entry, not a class — see [`STYLE.md`](../../STYLE.md), "Coordinator classes", which was mined from the framework tiers and explicitly doesn't govern endpoints.
