...
`SheetSchema` and `ColumnSchema` are **siblings**, both extending `SchemaBase`, as does `SpreadsheetSchema`. `ColumnSchema` does *not* extend `SheetSchema` — it reaches its sheet through a `sheet` accessor. That's forced: one accessor named `schema` declared at the Raw root means every narrowing override must be assignable to what the root declares, and the consuming class tree branches (the sheet class, the row base and the column base are siblings under a shared sheet-scoped base), so the two need only be assignable to `SchemaBase`, never to each other. The Raw root declares `schema` as `SchemaBase`, and each concrete spreadsheet class (`SpreadsheetRaw`, `SpreadsheetIndexed`, `SpreadsheetNamed`, `Api`) narrows it to `SpreadsheetSchema`.
