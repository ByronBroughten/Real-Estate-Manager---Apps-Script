...
**The Node host is the second one** (`src/nodeHost/`, launched by `scripts/nodeHost.mjs`). Spreadsheet I/O is three `RawSource` methods — fetch sheet properties, fetch grid ranges, apply the queued write list — plus one script property holding the spreadsheet ID. `GoogleSheetsAPI` maps those onto the three Advanced Service verbs and, in Node, an HTTP transport. `NodeHost.ensureGlobals()` installs a `PropertiesService` stub and a `Logger` onto the global scope, and injects `GoogleSheetsAPI` as the `RawSource`, before any framework module loads. It does **not** install a `Sheets` global, so a chore that reaches for `Sheets` or `SpreadsheetApp` still fails by name. Two commands use it: `npm run chore <name>` and `npm run gen:configs`.
