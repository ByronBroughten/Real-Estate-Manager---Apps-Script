...
**Apps Script is the production host.** `src/` is TypeScript compiled by `rollup` (via `@rollup/plugin-typescript`) into a single `dist/bundle.js`, which `clasp` pushes to a Google Apps Script project. The code then executes server-side inside Apps Script. Spreadsheet I/O goes through `GoogleSheetsAPI`, which is the only module that calls the Sheets Advanced Service (`Spreadsheets.get`, `getByDataFilter`, `batchUpdate`), not the `SpreadsheetApp` UI-bound API. Entry points are the top-level functions in `src/index.ts` — `triggerOnEdit` and `triggerOnChange`, which Apps Script calls by name from installed triggers — and each installs that adapter as the `RawSource` in Raw state before any framework work: `triggerOnChange` directly, `triggerOnEdit` still through `Api.init` until #104. Node/DOM APIs are not available there — only in the local build tooling (`rollup`, `tsc`, `scripts/*.mjs`).
