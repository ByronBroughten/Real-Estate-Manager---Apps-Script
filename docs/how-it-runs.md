...
`src/` is TypeScript compiled by `rollup` (via `@rollup/plugin-typescript`) into a single `dist/bundle.js`, which `clasp` pushes to a Google Apps Script project. The code then executes server-side inside Apps Script. Spreadsheet I/O goes through `GoogleSheetsAPI`, which is the only module that calls the Sheets Advanced Service (`Spreadsheets.get`, `getByDataFilter`, `batchUpdate`), not the `SpreadsheetApp` UI-bound API. Entry points are the top-level functions in `src/index.ts` — `triggerOnEdit` and `triggerOnChange`, which Apps Script calls by name from installed triggers — and each hands `Api`'s static handler the app's configs (`src/appConfigs.ts`) and endpoints. The handler installs the configs, which the tiers read through `Register`, and then that adapter as the `RawSource` in Raw state before any framework work. The Node host installs the same configs in `ensureGlobals`. `triggerOnEdit` decodes its event into a platform-neutral `SheetEdit` with `AppsScript.sheetEdit` and installs only once the edit looks like an action-row checkbox, so an ordinary edit costs no property read. `GoogleSheetsAPI.forAppsScript()` binds the adapter to the spreadsheet named by the `realEstateSpreadsheetId` script property, and throws naming that property when it is missing. Node/DOM APIs are not available there — only in the local build tooling (`rollup`, `tsc`, `scripts/*.mjs`).
