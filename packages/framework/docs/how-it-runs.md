...
- **The dev project needs two one-time human steps before a tick runs**: set its `realEstateSpreadsheetId` script property (the name is the framework's, whatever the app) to the dev spreadsheet's ID, and install installable triggers for `triggerOnEdit` (on edit) and `triggerOnChange` (on change). Then tick some rows' Selected box and the Result box in the action row of the `runItem` tab: the Run status column reads `Counted N selected row(s)`.
...
The bin's `probe` (`scripts/sheetsProbe.mjs`) sends one request through the Node host's `SheetsTransport`, authenticated like a chore. That request is a `GET` with a `fields` mask or a `:getByDataFilter`, and the script cannot build any other kind. It writes the full response, pretty-printed, to the gitignored `.probe/last.json` in its package: `.probe/last.json` for the app, `dev/.probe/last.json` for dev. Stdout gets only a summary: top-level keys, array counts, and each sheet's id and title. With `--path`, stdout gets that one subtree, printed whole when it is short and summarized when it is not. When the summary isn't enough, `Read` a line range of the file it names. The throwaway `scripts/*.tmp.mjs` route this replaced is retired.
