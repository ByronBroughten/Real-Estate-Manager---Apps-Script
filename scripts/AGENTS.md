...
- **`sheets-framework.mjs` is the only entry**: each subcommand is a module exporting a `run…` function that takes the loaded `sheets.config.json`, and none reads a spreadsheet ID any other way.
