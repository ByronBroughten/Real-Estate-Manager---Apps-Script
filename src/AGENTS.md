...
- **Dependencies point only downward, and lint enforces it.** A file goes in the lowest tier that satisfies it. `utils/` is below every tier; `businessEndpoints`, `chores/`, `nodeHost/` and `index.ts` sit above them all.
